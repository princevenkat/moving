...
export default function VendorDetails(
  ...
  const becomeVendor: FormEventHandler = (ev) => {
    ...
    post(route('vendor.store'),{
      ...
      onSuccess: () => {
        closeModel()
          setSuccessMessage('You can now get the inquiries from the users')
      },
      onError: () => {
  ...
  return (
    <section >
      ...
      <header>
        <h2 className='flex justify-between mb-8 text-lg font-medium text-gray-900 dark:text-gray-100'>Vendor Details</h2>
      ...
      <div>
        {!user.vendor && <PrimaryButton onClick={ev => setShowBecomeVendorConfirmation(true)} disabled={processing}>Become a Vendor</PrimaryButton>}
      ...
      <Modal show={showBecomeVendorConfirmation} onClose={closeModel}>
        <form onSubmit={becomeVendor} className='p-8'>
          ...
          <div className='mt-6 flex justify-end'>
            <SecondaryButton onClick={closeModel}>Cancel</SecondaryButton>
            <PrimaryButton className='ms-3' disabled={processing}>Confirm</PrimaryButton>
          </div>
